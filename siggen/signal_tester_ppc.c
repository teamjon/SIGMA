...
int main(void){
  ...
  for ( ; ; ){
    ...
    //fgets(ans, MAX_LINE, stdin);				//NOTE: changed rl_gets(ans, MAX_LINE); --> fgets(ans, MAX_LINE, stdin);
...
/* extract t20 (hit segment) and t80 (point contact) in units of 0.1 timesteps */
static int drift_t20_t80(float **s, int *t20, int *t80) {
  int   t, seg;
  float f1, f2;
  ...
  for (t=0; t < time_steps && s[seg][t]/s[seg][time_steps-1] < 0.2f; t++) ;
  f1 = s[seg][t-1]/s[seg][time_steps-1];
  f2 = s[seg][t  ]/s[seg][time_steps-1];
  *t20 = 10*(t-1) + lrintf(10.0f * (0.2f-f1)/(f2-f1));

  for (t=0; t < time_steps && s[0][t]/s[0][time_steps-1] < 0.8f; t++) ;
  f1 = s[0][t-1]/s[0][time_steps-1];
  f2 = s[0][t  ]/s[0][time_steps-1];
  *t80 = 10*(t-1) + lrintf(10.0f * (0.8f-f1)/(f2-f1));
...
static int save_signal(char *cmd){
  ...
  if (coord_type == CYL){
    if (sscanf(cmd, "%f %f %f", &cyl.r, &cyl.phi, &cyl.z) == 3
	  || sscanf(cmd, "%f, %f, %f", &cyl.r, &cyl.phi, &cyl.z) == 3){
      while (cyl.phi < 0) cyl.phi += 2.0*3.14159265;
      while (cyl.phi > 2.0*3.14159265) cyl.phi -= 2.0*3.14159265;
      cart = cyl_to_cart(cyl);
    }else{
      printf("error parsing coordinate: %s\n", cmd);
      return 1;
  ...
  /* find 20% time of hit segment, and 80% time of PC (seg 0) */
  if (drift_t20_t80(s, &t20, &t80) == 0)
    printf("*** t20-80 drift time = %d ns\n", t80-t20);
...
static int print_signal(char *cmd){
  ...
  if (dt < 0) {
    printf("\n\n Segment number being passed: %i\n\n",dt);
    printf("point not in crystal: (x = %.1f, y = %.1f, z = %.1f)\n",
	   cart.x, cart.y, cart.z);
    return 1;
  }

  /* When running for SIGMA fields, code breaks here if using s[seg][time_steps-1] > 0.9f,
     changed to -s[seg][time_steps-1] > 0.9f. The -ve sign returns the correct segment #
     due to the signals being -ve for SIGMA not +ve like LBNL PPC
     If sign is +ve, seg is set to 20 and seg fault when trying to access s[20][i]
  */
  for (seg = 1; seg < nsegments; seg++) {
    if (-s[seg][time_steps-1] > 0.9f) break;
  }

  printf("signal has drift time %d, hit segment %d\n", dt, seg);
  for (i = 0; i <= (dt+9)/10; i++){
    printf("%7.3f", s[seg][i]);
    if ((i+1)%10 == 0)
      printf("\n");
...
static int az_drift_time(char *cmd){
  ...
  for (phi=0; phi<361; phi+=5) {
    cyl.phi = 3.14159265 * (float) phi / 180.0;
    cart = cyl_to_cart(cyl);
    dt = get_signal(cart, s);
    if (dt < 0) {
    ...
    /* find 20% time of hit segment, and 80% time of PC (seg 0) */
    drift_t20_t80(s, &t20, &t80);
    printf("ang, dt = %3d %d %d\n", phi, dt, t80-t20);
    fprintf(fp, "%3d %d %d\n", phi, dt, t80-t20);
  }
...
static int drift_paths(char *cmd){
  ...
  for (i = 0; i < nt; i++){
    ...
    if (dpe[i].x == 0.0f && dpe[i].y == 0.0f && dpe[i].z == 0.0f &&
	dph[i].x == 0.0f && dph[i].y == 0.0f && dph[i].z == 0.0f) break;
  }
