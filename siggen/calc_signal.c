...
#include "Marco_fn.h"					// Contains Marco's method of calculating drift times for comp with exp data
#endif

#define HOLE_CHARGE 1.0
#define ELECTRON_CHARGE -1.0

int test_e_dt, test_h_dt;
...
/* GConCalculateDriftTime (MS)
   Calculates drift time using linear interpolation of signals
   used to compare SigGen drift times to Marco's experiemntal
   measurements - not currently working 30/08/16
*/
//  double Marco_dt = GConCalculateDriftTime(ntime_steps,(double*)signal[0][],(double*)signal[segment][],0.02,0.92,0.03,0);
//  printf("%d \n",(int)Marco_dt);

  drift_time = calc_dt(signal, 0.0, 0.95);
  return drift_time;
}

/* calc_dt (JW)
   Caclulates the drift time for the PC trace (signal) from
   lower to upper percentages as decimals
*/
static int calc_dt(float **signal, float lower, float upper) {
  int c1, c2, t1, t2, i;
  c1 = c2 = t1 = t2 = 0;

  for(i=1;i<ntime_steps;i++) {
    float test_sig = fabs(signal[0][i]);
...
static int make_signal(point pt, float **signal, float q){
  ...
  for (t = 0; (dv_res = drift_velocity(new_pt, q, &v)) >= 0; t++){
    if (q > 0) {
      ...
      dpath_e[t] = new_pt;
      test_e_dt++;			// drift time for electron only
    }
