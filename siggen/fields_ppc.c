...
#undef POW6
  sign = (q < 0 ? -1 : 1);
  *velo = vector_scale(cart_en, sign*absv);
...
*/
static int setup_velo(char *fname){
  static char velo_fname[MAX_FNAME_LEN];
  char line[MAX_LINE];
  FILE *fp;
  ...
  // save the file name so that we can re-read velocities if we change the detector temperature later...
  if (fname) strncpy(velo_fname, fname, sizeof(velo_fname));
  ...
  for (v_lookup_len = 0; ;v_lookup_len++){
    ...
    if (sscanf(line, "%f %f %f %f %f %f %f",
	       ...
	       &v_lookup[v_lookup_len].h111) != 7) break;//asume EOF
    ...
    } else if (0) { // modified scale to v110
      if (v_lookup[v_lookup_len].e < 999) {
	v_lookup[v_lookup_len].e100 = v_lookup[v_lookup_len].e110 * 10.1f/9.5f;
	v_lookup[v_lookup_len].e111 = v_lookup[v_lookup_len].e110 *  8.0f/9.5f; // note 8.0
      }
    } else if (0) { // a different modified scale to v110
      if (v_lookup[v_lookup_len].e < 999) {
	v_lookup[v_lookup_len].e100 = v_lookup[v_lookup_len].e110 * 9.9f/9.5f; // note 9.9
	v_lookup[v_lookup_len].e111 = v_lookup[v_lookup_len].e110 * 7.6f/9.5f; // note 7.6
      }
    } else if (0) { // scale to v111
      if (v_lookup[v_lookup_len].e < 999) {
	v_lookup[v_lookup_len].e100 = v_lookup[v_lookup_len].e111 * 10.1f/7.78f;
	v_lookup[v_lookup_len].e110 = v_lookup[v_lookup_len].e111 *  9.5f/7.8f;
      }
    }
    /*  ------------------------ */
    tell(ANNOYINGLY_CHATTY, "%10.3f%8.3f%8.3f%8.3f%8.3f%8.3f%8.3f\n",
	 tmp->e, tmp->e100, tmp->e110, tmp->e111, tmp->h100, tmp->h110, tmp->h111);
    if (read_setup_line(fp, line) != 0) break;   // no more data in file
    if (line[0] == 'e' || line[0] == 'h') break; // no more velocities, now have temp corr data
  }
  ...
  while (line[0] == 'e' || line[0] == 'h') {
    ...
    if (line[0] == 'e')
      tell(ANNOYINGLY_CHATTY,
	   "electrons: mu_0 = %.2e x T^%.4f  B = %.2e  Theta = %.0f\n",
	   mue, pwre, be, thetae);
    if (line[0] == 'h')
      tell(ANNOYINGLY_CHATTY,
	   "    holes: mu_0 = %.2e x T^%.4f  B = %.2e  Theta = %.0f\n",
	   muh, pwrh, bh, thetah);

    if (read_setup_line(fp, line) != 0) break;
  }
  ...
  */
  ...
  tell(ANNOYINGLY_CHATTY, "Index  field  vel_factor\n");
  mu_0_1 = mue * pow(REF_TEMP, pwre);
  v_s_1 = be * sqrt(tanh(0.5 * thetae / REF_TEMP));
  E_c_1 = v_s_1 / mu_0_1;
  mu_0_2 = mue * pow(temperature, pwre);
  v_s_2 = be * sqrt(tanh(0.5 * thetae / temperature));
  E_c_2 = v_s_2 / mu_0_2;
  for (i = 0; i < vlook_sz; i++){
    ...
    v_lookup[i].e110 *= f;
    v_lookup[i].e111 *= f;
    tell(ANNOYINGLY_CHATTY, "%2d %5.0f %f\n", i, e, f);
  }
  ...
  E_c_1 = v_s_1 / mu_0_1;
  mu_0_2 = muh * pow(temperature, pwrh);
  v_s_2 = bh * sqrt(tanh(0.5 * thetah / temperature));
  E_c_2 = v_s_2 / mu_0_2;
  for (i = 0; i < vlook_sz; i++){
    e = v_lookup[i].e;
    if (e < 1) continue;
  ...
  //sumb_e = -10;  // FIXME Fudge by DCR    original values
  //sumc_e =  5;   // FIXME Fudge by DCR

  //sumb_e = -2;    //  - closer to straight large r, further at small r
  //sumc_e =  -1;   //       large z => further at all r, same as (0, -3) below
  //sumb_e = -3.5;  //  - slightly closer at small r?
  //sumc_e =  0;    //       large z =>further at all r, but closer than (0, -3) below
  //sumb_e =  0;    //  - closer at all r, except curves back at intermediate r
  //sumc_e =  -3;   //       large z => further at all r
  //sumb_e =  0;    //   - bends out, then in!
  //sumc_e =  -6;   //       large z => path close to straight, good dt

  sumb_e = -15;   // FIXME Fudge by DCR  - gives good results for ascan5 times and decomp phi
  sumc_e = 7.5;   // FIXME Fudge by DCR
  sumb_e = -10;  // FIXME Fudge by DCR    original values
  sumc_e =  5;   // FIXME Fudge by DCR
  sumb_e = sumc_e = sumb_h = sumc_h = 0.0;
