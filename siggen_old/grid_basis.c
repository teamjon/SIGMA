...
int main(void) {
  float r, p, z, zz, f1, f2;
  int   i, seg, seg2, t, t80, t20, old_seg, n=0, longest_drift=0;
  struct point cart;
  struct cyl_pt cyl;
  static float **s;
  FILE   *fp;
  ...
  for (p = 0.0f; p < M_PI/4.0 + 0.001; p += P_STEP) {
    bp.ip++;
    cyl.phi = p;
    bp.ir = -1;
	...
	if (seg >= 0 && seg < nsegments) {
	  ...
	  bp.t80 = t80;
	  bp.t20 = t20;
	  for (t=0; t<TIME_STEPS_C; t++) bp.signal[0][t] = -s[0][t];  // invert neg PC signal
	  for (i=1; i<NUM_SIGS; i++) {
	    for (t=0; t<TIME_STEPS_C; t++) bp.signal[i][t] = s[i][t];
	  }
